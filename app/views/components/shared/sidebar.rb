...
class Shared::Sidebar < ApplicationComponent
    ...
    def component_link(component)
        ...
        return a(href: component[:path], class: tokens('group flex w-full items-center rounded-md border border-transparent px-2 py-1 hover:underline', -> { current_path } => "text-foreground", -> { !current_path } => "text-muted-foreground")) { component[:name] }
    end
end
