    ...
    header(class: 'supports-backdrop-blur:bg-background/60 sticky top-0 z-50 w-full border-b bg-background/95 backdrop-blur') do
      div(class: 'container flex h-14 items-center justify-between') do
        ...
        div(class: 'flex items-center') do
          render Link.new(href: "https://github.com/georgekettle/PhlexUI", variant: :ghost, size: :icon)  do
          ...
          div(data: { controller: 'toggle-theme' }) do
            # Light Mode
            render Button.new(href: "#", variant: :ghost, size: :icon, class: 'dark:hidden', data: { action: 'click->toggle-theme#setDarkTheme' }) do
              svg(
            ...
            # Dark Mode
            render Button.new(href: "#", variant: :ghost, size: :icon, class: 'hidden dark:flex', data: { action: 'click->toggle-theme#setLightTheme' }) do
              svg(
                xmlns: "http://www.w3.org/2000/svg",
                viewbox: "0 0 24 24",
                fill: "currentColor",
