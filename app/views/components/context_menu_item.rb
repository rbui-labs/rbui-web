...
class ContextMenuItem < ApplicationComponent
    def initialize(href: "#", **attributes)
        @attributes = attributes
        @href = href
        @checked = attributes.delete(:checked) || false
        @shortcut = attributes.delete(:shortcut)
    ...
    def template(&)
        a(
            **@attributes,
            ...
            role: "menuitem",
            class:
                "relative flex cursor-pointer select-none items-center rounded-sm px-2 py-1.5 text-sm outline-none hover:bg-accent hover:text-accent-foreground focus:bg-accent focus:text-accent-foreground data-[disabled]:pointer-events-none data-[disabled]:opacity-50 pl-8",
            tabindex: "-1",
